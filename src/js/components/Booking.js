...
class Booking{
  ...
  initWidgets(){
    ...
    thisBooking.dom.starters.addEventListener('click', function(event){
      ...
      //console.log(name, type, tagName, value);

      if (name == 'starter' && type == 'checkbox' && tagName == 'INPUT'){

        if (event.target.checked){
          thisBooking.starters.push(value);
        }else{
          const index = thisBooking.starters.indexOf(value);
  ...
  initTables(event){
    ...
    if(event.target.classList.contains('table')){
      ...
      const tableId = parseInt(thisTable.dataset.table);
      const bookedNow = thisBooking.booked[thisBooking.date][thisBooking.hour];
     
     
      if(bookedNow.includes(tableId)){
        window.alert('This table is occupied');
      }
        ...
        for(let table of thisBooking.dom.tables){
          ...
          }else if(table.dataset.table == tableId){
            if(table.classList.contains('selected')){
              table.classList.remove('selected');
              thisBooking.selectedTable = null;
            }else {
  ...
  resetTables(){
    ...
    for(const table of thisBooking.dom.tables){
      if(table.classList.contains('selected')){
        table.classList.remove('selected');
        thisBooking.selectedTable = 'Not defined';
      }
    }
  }
  ...
  sendBooking(){
    ...
    payload.duration = thisBooking.amountWidgetHours.correctValue;
    payload.ppl = thisBooking.amountWidgetPeople.correctValue;
